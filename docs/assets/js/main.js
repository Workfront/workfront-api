        ...
        var EventTarget = (function () {
            ...
            return (
                ...
                (EventTarget.prototype.dispatchEvent = function (event) {
                    ...
                    for (
                        var stack = this.listeners[event.type].slice(), i = 0, l = stack.length;
                        i < l;
                        i++
                    )
                    ...
                    ;-1 != base.indexOf('#') && (base = base.substr(0, base.indexOf('#'))),
                        this.el.querySelectorAll('a').forEach(function (el) {
                            ...
                            if (-1 != href.indexOf('#') && href.substr(0, base.length) == base) {
                                ...
                                anchor &&
                                    link &&
                                    _this.anchors.push({link: link, anchor: anchor, position: 0})
                            }
                        }),
                        this.onResize()
                }),
                ...
                return (
                        ...
                        if (
                            ...
                            for (var i = 0, c = Math.min(10, res.length); i < c; i++) {
                                var row = this.data.rows[Number(res[i].ref)],
                                    name = row.name.replace(new RegExp(this.query, 'i'), function (
                                        match
                                    ) {
                                        return '<b>' + match + '</b>'
                                    }),
                    ...
                    (Search.prototype.setCurrentResult = function (dir) {
                        ...
                        if (current) {
                            ...
                            rel &&
                                (current.classList.remove('current'), rel.classList.add('current'))
                        } else
                    ...
                    (Search.prototype.gotoCurrentResult = function () {
                        var current = this.results.querySelector('.current')
                        if (
                            (current || (current = this.results.querySelector('li:first-child')),
                            current)
                        ) {
